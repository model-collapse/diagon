...
Lucene105PostingsWriter::Lucene105PostingsWriter(index::SegmentWriteState& state)
    : docOut_(nullptr)
    , skipOut_(nullptr)
    ...
    // Create .skp output file (for skip entries)
...
Lucene105PostingsWriter::~Lucene105PostingsWriter() {
    if (docOut_ || skipOut_) {
        try {
            close();
        } catch (...) {
...
void Lucene105PostingsWriter::setField(const index::FieldInfo& fieldInfo) {
    indexOptions_ = fieldInfo.indexOptions;
    writeFreqs_ = (indexOptions_ >= index::IndexOptions::DOCS_AND_FREQS);
}

void Lucene105PostingsWriter::startTerm() {
...
void Lucene105PostingsWriter::startDoc(int docID, int freq, int8_t norm) {
    if (docID < 0) {
...
void Lucene105PostingsWriter::maybeFlushSkipEntry() {
...
TermState Lucene105PostingsWriter::finishTerm() {
...
void Lucene105PostingsWriter::writeSkipData() {
...
void Lucene105PostingsWriter::flushBuffer() {
    if (bufferPos_ != BUFFER_SIZE) {
...
int64_t Lucene105PostingsWriter::getFilePointer() const {
    return docOut_ ? docOut_->getFilePointer() : 0;
}

std::vector<uint8_t> Lucene105PostingsWriter::getDocBytes() const {
...
std::vector<uint8_t> Lucene105PostingsWriter::getSkipBytes() const {
