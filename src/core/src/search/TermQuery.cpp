...
public:
    TermScorer(const Weight& weight,
...
public:
    TermWeight(const TermQuery& query,
...
private:
    static BM25Similarity::SimScorer createScorer(
...
std::unique_ptr<Weight> TermQuery::createWeight(
    IndexSearcher& searcher,
    ScoreMode scoreMode,
    float boost) const {
